...
paises <- read.csv("paises.csv", stringsAsFactors = FALSE, sep = ";", header = TRUE, fileEncoding = "latin1")
...
MapaBase <- readRDS("maps/gadm41_AFG_1_pk.rds") %>% st_as_sf()
  ...
  sidebarLayout(#Panel de herramientas lateral
    ...
    sidebarPanel(
      ...
      radioButtons(
        inputId = "opcion_bio",
        label = "Opciones:",
        choices = c("Colours", "Legend", "Elevations","Rivers", "nivel_solicitado = "),
        selected = "Colours"
      ),
    ...
    sidebarLayout(
      sidebarPanel(
        selectInput("especie", "Selecciona una especie",
          choices = c("Quercus", "Quercus robur", "Quercus ilex", "Quercus pirenaica")),
        actionButton("buscar_especie", "Buscar por Especie")
      ),
      mainPanel(
        plotOutput("grafico")  # Agrega un elemento de salida para el gráfico
      )
    )
    ), #Fin de sidebarPanel
...
descarga_mapa <- function(pais = "", nivel_solicitado){

  #Busca el nivel mayir de carga d mapas
  esta_mapa_local <- esta_mapa_en_local(pais,nivel_solicitado) #Busca si el archivo esta en local con cierto nivel

  # Si el archivo de ese mapa con ese nivel no esta en local, y esta disponible, solicitalo a internet
  if (esta_mapa_local == "vacio") {
...
esta_mapa_en_local <- function(pais="",nivel){
  ...
  map_file41 <- paste0("maps/gadm41_",pais, "_", nivel, "_pk.rds")
  map_file36 <- paste0("maps/gadm36_",pais, "_", nivel, "_sp.rds")
...
server <- function(input, output,session){
  ...
  observeEvent(input$seleccion_pais, {
    ...
    descarga_mapa(pais_seleccionado2, nivel_solicitado )
  ...
  observeEvent(input$reset, {
    ...
    nivel_solicitado = 1
    cambia_fill_rows(nivel_solicitado) # La leyenda depende de este valor
    descarga_mapa("AFG", nivel_solicitado )
    MapaBase <- React_MapaBase()
